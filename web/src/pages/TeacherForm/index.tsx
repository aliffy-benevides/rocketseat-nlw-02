...
function TeacherForm() {
  ...
  function addNewScheduleItem() {
    if (scheduleItems.length < 7) {
      const newWeekDay = Array.from(Array(7).keys()).reduce((previous, current) => {
        if (previous === -1) {
          // If not exist current item in array
          if (!scheduleItems.find(item => Number(item.week_day) === current)) {
            return current;
          }
        }
  ...
  function setScheduleItemValue(index: number, field: string, value: string) {
    const updatedScheduleItems = scheduleItems.map((item, i) => {
      if (i === index) {
        // If the altered field is the week_day and it already exists one item with the same week_day, do not change item
        if (field === 'week_day' && scheduleItems.find(actual => Number(actual.week_day) === Number(value))) {
          return item;
        }
